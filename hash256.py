# -*- coding: utf-8 -*-
"""
Created on Tue Dec  5 09:29:10 2023
...
#def rot_left(a: int, b: int):
#    return (a << b) | (a >> (32 - b))

def rot_right(a: int, b: int):
    return (a >> b) | (a << (32 - b))
...
def sig0(x: int):
    return rot_right(x, 17) ^ rot_right(x, 18) ^ (x >> 3)

def sig1(x: int): 
    return rot_right(x, 17) ^ rot_right(x, 19) ^ (x >> 10)

def generate_hash(txt: str):
    byte_array = bytearray()
    byte_array.extend(txt)
    ...
    for i in range(0, len(byte_array), 64): 
        blocks.append(byte_array[i:i+64])
